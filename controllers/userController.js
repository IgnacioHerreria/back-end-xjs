...
var userController = {
    ...
    UpdateUser: (req, res) => {
        ...
        if (validatorMail && validatorRol && validatorState) {
            userModel.findByIdAndUpdate({ _id: id }, { params }, { new: true, runValidators: true },
                (err, userUpdated) => {
                    if (err) {
                        return res.status(500).json({
                            ok: false,
    ...
    UploadImage: (req, res) => {
        ...
        if (
            ...
            userModel.findOneAndUpdate({ _id: idUser }, { image: file_name }, { new: true, runValidators: true },
                (err, userUpdated) => {
                    if (err) {
                        return res.status(500).json({
                            ok: false,
